...
import { 
  Camera, FileSpreadsheet, Cpu, CheckCircle2, Play, RefreshCw, X, Award, Sparkles, Image as ImageIcon, Scan, ArrowRight
} from 'lucide-react';
...
export const RegisterOCRModal: React.FC<RegisterOCRModalProps> = ({ isOpen, onClose }) => {
  ...
  const [selectedSample, setSelectedSample] = useState<'lh1' | 'lh2' | 'lh3'>('lh1');
  ...
  const handleStartOCR = () => {
    ...
    setTimeout(() => {
      ...
      const csvRowsText = currentSample.rows.map(r => 
    ...
    setTimeout(() => {
      const updatedMetrics = mlSurplusPredictor.trainModel(currentSample.rows);
      ...
      <div className="w-full max-w-4xl bg-[#0e111a] border border-amber-500/40 rounded-3xl p-5 sm:p-7 shadow-2xl text-slate-100 space-y-5 relative overflow-hidden max-h-[92vh] overflow-y-auto">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="font-bold text-base sm:text-lg text-amber-100">Handwritten Mess Register Image OCR Digitizer</h2>
                <span className="px-2.5 py-0.5 rounded-full bg-amber-500/20 text-amber-300 text-[10px] font-mono font-bold border border-amber-500/30 uppercase">
                  PAPER-TO-ML PIPELINE
                </span>
              </div>
              <p className="text-xs text-slate-400 font-mono">
                Snap/Upload paper register image $\rightarrow$ AI OCR extracts CSV $\rightarrow$ Retrains ML Model live!
              </p>
            </div>
          </div>
        ...
        <div className="space-y-2">
          <div className="flex justify-between text-xs font-mono">
            <span className="text-amber-300 font-bold">
              {step === 1 && "Step 1: Select or Upload Handwritten Register Image"}
              {step === 2 && "Step 2: AI OCR Scanning & Handwriting Recognition..."}
              {step === 3 && "Step 3: Extracting Table Data to CSV Format..."}
              {step === 4 && "Step 4: ML Model Retrained on Register Data!"}
            </span>
            ...
            {(Object.keys(samples) as Array<keyof typeof samples>).map((key) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="text-[10px] text-slate-400 mt-0.5">Hostel {sm.hostel} • {sm.rows.length} Log Entries</div>
                </button>
              );
            })}
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="relative rounded-2xl overflow-hidden border border-amber-500/30 bg-black h-56 flex items-center justify-center">
            ...
            <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between text-[11px] font-mono bg-black/80 px-3 py-1.5 rounded-xl border border-white/10 backdrop-blur-md">
              <span className="text-amber-300 font-bold truncate">{currentSample.name}</span>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between text-xs font-mono font-bold text-slate-300 border-b border-slate-800 pb-2">
                <span className="flex items-center gap-1.5">
                  ...
                  <span>AI OCR Extracted CSV Output</span>
                </span>
              ...
              {step >= 3 ? (
                ...
                <div className="h-36 flex flex-col items-center justify-center text-slate-500 font-mono text-xs space-y-2 text-center p-4 border border-dashed border-slate-800 rounded-xl">
                  ...
                  <p>Click "Run AI OCR Scan" to convert paper register photo into CSV!</p>
                </div>
              )}
            </div>
        ...
        <div className="p-4 rounded-2xl bg-slate-950 border border-amber-500/30 space-y-2">
          ...
          <p className="text-xs text-slate-200 font-sans italic leading-relaxed bg-black/60 p-3 rounded-xl border border-white/5">
            "For hostel dining halls with zero computer hardware, mess staff simply snap a photo of their handwritten paper register. Our **AI OCR Digitization Engine** extracts the handwritten numbers into structured CSV data, feeds it directly into our ML regression model, and predicts tomorrow's kitchen prep quantity!"
          </p>
        </div>
        ...
        <div className="flex items-center justify-between pt-2 border-t border-amber-500/20">
          <div className="text-xs font-mono text-slate-400">
            Pipeline Status: <span className="text-amber-300 font-bold">{step === 4 ? "Complete (Paper -> CSV -> ML)" : "Ready to Scan"}</span>
          </div>
          ...
          >
            {isScanning ? (
              <>
                ...
                <span>Running AI OCR Scan...</span>
              </>
            ) : (
              <>
                <Scan className="w-4 h-4 text-black" />
                <span>RUN AI OCR SCAN (REGISTER -&gt; CSV -&gt; ML)</span>
              </>
            )}
          </button>
        </div>
